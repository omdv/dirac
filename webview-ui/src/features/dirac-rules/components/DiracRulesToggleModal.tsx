...
const DiracRulesToggleModal: React.FC = () => {
	const {
	...
	const toggleRule = (isGlobal: boolean, rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleDiracRule(
			ToggleDiracRuleRequest.create({
			...
			.catch((error) => {
				console.error("Error toggling Dirac rule:", error)
			})
	}
	...
	const toggleWindsurfRule = (rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleWindsurfRule(
			...
			.then((response: DiracRulesToggles) => {
				if (response.toggles) {
	...
	return (
		<div className="inline-flex min-w-0 max-w-full items-center" ref={modalRef}>
			<div className="inline-flex w-full items-center" ref={buttonRef}>
				<Tooltip>
					{!isVisible && <TooltipContent>Manage Dirac Rules & Workflows</TooltipContent>}
					<TooltipTrigger>
						<VSCodeButton
							appearance="icon"
							aria-label={isVisible ? "Hide Dirac Rules & Workflows" : "Show Dirac Rules & Workflows"}
					...
					<div className="flex-shrink-0 px-2 pt-0">
						...
						<div className="text-xs text-description mb-4">
							{currentView === "rules" ? (
								...
								<p>
									Skills are reusable instruction sets that Dirac can activate on-demand. When a task matches a
									skill's description, Dirac uses the <span className="font-bold">use_skill</span> tool to load
									the full instructions.
								</p>
							) : (
								<p>
									Hooks allow you to execute custom scripts at specific points in Dirac's execution lifecycle,
...
export default DiracRulesToggleModal
